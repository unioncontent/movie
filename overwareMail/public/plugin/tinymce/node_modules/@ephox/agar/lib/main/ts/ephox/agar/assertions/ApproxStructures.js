...
var assertChildren = function (expectedChildren, actual) {
    expectedChildren.each(function (expected) {
        ...
        RawAssertions.assertEq('Checking number of children of: ' + Truncate.getHtml(actual) + '\nComplete Structure: \n' + Html.getOuter(actual), expected.length, children.length);
        Arr.each(children, function (child, i) {
            var exp = expected[i];
            if (exp.doAssert === undefined)
                throw new Error(Json.stringify(exp) + ' is not a *structure assertion*.\nSpecified in *expected* children of ' + Truncate.getHtml(actual));
            exp.doAssert(child);
        });
    });
};
