...
UnitTest.test('Atomic Test: api.ValueSchemaFuncTest', function () {
    ...
    checkRawResultIs('Trim an argument, no postprocess', 'a.b', function (f) {
        return f('a', 'b', 'c');
    }, getter1, ValueSchema.funcOrDie(['a', 'b'], ValueSchema.anyValue()));
    checkRawErrIs('Checking if validation fails in postprocessing', 'wrong value', function (f) {
        return f('x');
    }, function (v) {
        return 'y';
    ...
    checkRawResultIs('Running a schema on the output', {
        A: 'A-value'
    }, function (f) {
        return f('A');
    }, getter2, ValueSchema.funcOrDie(['a'], ValueSchema.objOf([
    ...
    checkRawResultIs('Truncating output and passing through undefined', [
    ...
    }, getter3, ValueSchema.funcOrDie(['one', 'two'], ValueSchema.arrOfObj([
        FieldSchema.strict('firstname'),
        FieldSchema.strict('surname')
    ])));
});
//# sourceMappingURL=ValueSchemaFuncTest.js.map