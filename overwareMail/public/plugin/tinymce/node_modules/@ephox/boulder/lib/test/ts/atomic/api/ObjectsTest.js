...
UnitTest.test('ObjectsTest', function () {
    ...
    var testConsolidate = function () {
        ...
        checkErr('[ failure ] ]', ['failure.1'], [
            Result.error(['failure.1'])
        ], {});
        checkErr('[ failure {1,2,3} ]', ['failure.1', 'failure.2', 'failure.3'], [
            Result.error(['failure.1']),
            Result.error(['failure.2']),
            Result.error(['failure.3'])
        ], {});
        checkVal('[ value.a ]', { a: 'A' }, [
            Result.value({ a: 'A' })
        ], {});
        checkVal('[ value.{a,b,c} ]', { a: 'A', b: 'B', c: 'C' }, [
            Result.value({ a: 'A' }),
            Result.value({ b: 'B' }),
            Result.value({ c: 'C' })
        ], {});
        checkErr('[ value.a, failure.1, value.b ]', ['failure.1'], [
            Result.value({ a: 'A' }),
            Result.error(['failure.1']),
            Result.value({ b: 'B' })
        ], {});
        checkErr('[ value.a, failure.1, value.b, failure.2, failure.3 ]', ['failure.1', 'failure.2', 'failure.3'], [
            Result.value({ a: 'A' }),
            Result.error(['failure.1']),
            Result.value({ b: 'B' }),
            Result.error(['failure.2']),
        ...
        checkErr('[ value.{a,b,c} ]', ['failure.last'], [
            Result.value({ a: 'A' }),
            Result.value({ b: 'B' }),
            Result.value({ c: 'C' }),
        ...
        check('Testing consolidate', inputList, function (input) {
            ...
            if (hasError) {
                assert.eq(true, actual.isError(), 'Error contained in list, so should be error overall');
            }
            else {
        ...
        Jsc.syncProperty('Testing consolidate with base', [inputList, Jsc.nestring, Jsc.json], function (input, baseKey, baseValue) {
