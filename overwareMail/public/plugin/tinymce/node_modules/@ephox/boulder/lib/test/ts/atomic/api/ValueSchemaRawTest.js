...
UnitTest.test('ValueSchemaRawTest', function () {
    ...
    checkErr('test.3 (with extra and only) and two fields', 'unsupported fields: [c]', {
        a: 'a',
        b: 'b',
        c: 'c'
    ...
    checkErr('test.3 (with extra and only) and no fields', 'unsupported fields: [aa]', {
        aa: 'aa'
    }, ValueSchema.objOfOnly([]));
    check('test.3 with no forbidden field', {
        a: 'a',
        b: 'b'
    }, ValueSchema.objOfOnly([
        FieldSchema.strict('a'),
    ...
    checkErr('test.3 with 1 forbidden field', 'Do not use c. Use b', {
        a: 'a',
        b: 'b',
        c: 'c'
    }, ValueSchema.objOf([
    ...
    checkErr('test.6 should fail because fields do not both start with f', 'start-with-f', {
        fieldA: { val: 'a' },
        cieldB: { val: 'b' }
    ...
    checkErr('test.6b should fail because values do not contain "val"', 'val', {
        fieldA: { val2: 'a' },
        fieldB: { val2: 'b' }
    ...
    Logger.sync('asDefaultedOptionThunk supplied as true', function () {
        ...
        RawAssertions.assertEq('Checking output', 'defaulted thunk.fallback', v.alpha.getOrDie('Alpha should be some'));
    });
    Logger.sync('asDefaultedOptionThunk supplied', function () {
        ...
        RawAssertions.assertEq('Checking output', 'alpha.value', v.alpha.getOrDie('Alpha should be some'));
    });
    Logger.sync('mergeWithThunk({ extra: s.label }), value supplied', function () {
        var v = ValueSchema.asRawOrDie('test.mergeWith', ValueSchema.objOf([
    ...
    Logger.sync('Checking choose', function () {
        var processor = ValueSchema.choose('type', {
            general: [
        ...
        checkIs('Checking choose(other)', { houses: '10' }, {
            type: 'other',
            houses: '10'
        }, processor);
        checkErr('Checking choose(other) without everything', 'Could not find valid *strict* value for "houses"', {
            type: 'other',
            house: '10'
        }, processor);
        checkErr('Checking choose(other) without wrong schema', 'Could not find valid *strict* value for "houses"', {
            type: 'other',
            cards: '10'
        }, processor);
        checkIs('Checking choose(general)', { cards: '10' }, {
            type: 'general',
            cards: '10'
        }, processor);
        checkErr('Checking choose(general) without everything', 'Could not find valid *strict* value for "cards"', {
            type: 'general',
            mech: '10'
        }, processor);
        checkErr('Checking choose(general) with wrong schema', 'Could not find valid *strict* value for "cards"', {
            type: 'general',
            houses: '10'
        }, processor);
        checkErr('Checking choose(dog)', 'chosen schema: "dog" did not exist', {
            type: 'dog',
            houses: '10'
        }, processor);
    });
    ...
    Logger.sync('asRaw with type', function () {
        ...
        ValueSchema.asRaw('SomeType', schema, {
            num: 42,
            str: 'a'
        }).fold(function () { return assert.fail('Should not fail'); }, function (actual) { return RawAssertions.assertEq('Should be expected object', {
            num: 42,
            str: 'a'
        }, actual); });
        ValueSchema.asRaw('SomeType', schema, {}).fold(function (err) { return RawAssertions.assertEq('Should be two errors', 2, err.errors.length); }, function (actual) { return assert.fail('Should not pass'); });
    });
});
//# sourceMappingURL=ValueSchemaRawTest.js.map