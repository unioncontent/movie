...
UnitTest.test('ClusteringTest', function () {
    ...
    var check = function (label, universe, expLeft, expMiddle, expRight, expLang, id) {
        Logger.sync(id + ' => check: ' + label, function () {
            ...
            RawAssertions.assertEq('start: ' + id + ', check lang(): expected: ' + expLang.toString() + ', actual: ' + act.lang().toString(), true, expLang.equals(act.lang()));
    ...
    var testSanity = function () {
        ...
        check('Left: stopped by p2, middle: itself, right: stopped by p1.span2 lang', universe, [], ['z3a'], ['z3b', 'z3c'], Option.some('DE'), 'p2.text1.id');
        check('Left: stopped by p2, middle: itself, right: stopped by p1.span2 lang', universe, ['z3a'], ['z3b'], ['z3c'], Option.some('DE'), 'p2.text2.id');
        check('Left: stopped by p2, middle: itself, right: stopped by p1.span2 lang', universe, ['z3b', 'z3a'], // intentionally ordered that way for "left" call, but not "all"
        ['z3c'], [], Option.some('DE'), 'p2.span1.text1.id');
        check('Left: stopped by p2.span2 lang, middle: itself, right: stopped by p2.span2 lang', universe, [], ['z4a'], [], Option.some('FR'), 'p2.span2.text1.id');
        check('Left: stopped by p3, middle: itself, right: stopped by space in "g and"', universe, [], [' '], ['do', 'g'], Option.none(), 'p3.text1.id');
        check('Left: stopped by " ", middle: itself, right: stopped by space in "g and"', universe, [], ['do'], ['g'], Option.none(), 'p3.text2.id');
        check('Left: stopped by " ", middle: itself, right: stopped by space in " bone"', universe, ['do'], ['g and'], [], Option.none(), 'p3.text3.id');
        check('Left: stopped by space in "g and", middle: itself, right: stopped by p3', universe, ['and'], [' bone'], [], Option.none(), 'p3.text4.id');
    };
    var checkProps = function (universe, textIds, start, actual) {
        var checkGroup = function (label, group) {
            ...
            Arr.each(items, function (x, i) {
                ...
                RawAssertions.assertEq('Check that everything in the ' + label + ' is a text node', true, universe.property().isText(x));
            });
        };
        ...
        Arr.each(actual.all(), function (x, i) {
            if (i > 0) {
                ...
                RawAssertions.assertEq('The text nodes should be one after the other', +1, textIds.indexOf(current) - textIds.indexOf(prev));
            }
        });
        var blockParent = universe.up().predicate(start, universe.property().isBoundary).getOrDie('No block parent tag found');
        Arr.each(actual.all(), function (x, i) {
            RawAssertions.assertEq('All block ancestor tags should be the same as the original', blockParent, universe.up().predicate(x.item(), universe.property().isBoundary).getOrDie('No block parent tag found'));
        });
    };
    var propertyTest = function (label, universe) {
        Logger.sync(label, function () {
            Jsc.property(label + ': Checking that text nodes have consistent zones', Arbitraries.arbTextIds(universe), function (idInfo) {
    ...
    testSanity();
    propertyTest('Testing with no languages at all', TestUniverse(Gene('root', 'root', [
        Gene('p1', 'p', [
    ...
    propertyTest('Testing with no root language', TestUniverse(Gene('root', 'root', [
        Gene('p1', 'p', [
            Gene('p1s1', 'span', [
                TextGene('p1sp', 'sp'),
                Gene('p1s2', 'span', [
    ...
    propertyTest('Testing with DE as root language', TestUniverse(Gene('root', 'root', [
        Gene('p1', 'p', [
            Gene('p1s1', 'span', [
                TextGene('p1sp', 'sp'),
                Gene('p1s2', 'span', [
...
//# sourceMappingURL=ClusteringTest.js.map