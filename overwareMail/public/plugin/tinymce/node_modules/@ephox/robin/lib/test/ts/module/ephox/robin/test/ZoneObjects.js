    ...
    Arr.each(zones, function (zone) {
        ...
        Logger.sync('\nProperty test for zone: ' + Json.stringify(rawOne(universe, zone), null, 2), function () {
            ...
            Arr.each(elements, function (x, i) {
                RawAssertions.assertEq('Checking everything in ' + label + ' has same language. Item: ' + x.id, LanguageZones.calculate(universe, x).getOr('none'), zone.lang());
            ...
            Arr.each(elements, function (x, i) {
                RawAssertions.assertEq('All block ancestor tags should be the same as the original', blockParent, universe.up().predicate(x, universe.property().isBoundary).getOrDie('No block parent tag found'));
            });
        });
    });
};
...
//# sourceMappingURL=ZoneObjects.js.map