...
UnitTest.test('TextZoneTest', function () {
    ...
    var checkZone = function (label, expected, actual) {
        expected.fold(function () {
            actual.fold(function () {
                ...
                assert.fail(label + '\nShould not have created zone: ' + Json.stringify(Json.stringify(ZoneObjects.rawOne(doc1, act))));
            });
        }, function (exp) {
            actual.fold(function () {
    ...
    checkSingle('Basic zone for one text field', Option.some({
        lang: 'en',
        words: ['one'],
        elements: ['en-a']
    }), 'en-a', 'en');
    checkSingle('Basic zone for isolated span should be none because no text nodes inside', Option.none(), 'span-isolated', 'en');
    checkSingle('Basic zone for semi isolated span should have the partial words outside it', Option.some({
        lang: 'en',
        words: ['on'],
        elements: ['en-j', 'en-k']
    }), 'span-semi-isolated', 'en');
    checkRange('Basic ranged zone for two adjacent english text nodes should create zone with them', Option.some({
        lang: 'en',
        words: ['two'],
        elements: ['en-b', 'en-c']
    }), 'en-b', 'en-c', 'en');
    checkRange('Basic ranged zone for an english text node next to another one (but not part of the range) should create zone with them', Option.some({
        lang: 'en',
        words: ['two'],
        elements: ['en-b', 'en-c']
    }), 'en-b', 'en-b', 'en');
    checkRange('Basic ranged zone for an english text node to a german text node should create no zone', Option.none(), 'en-b', 'de-a', 'en');
    ...
    PropertyAssertions.check('Check text single', [
        Arbitraries.arbIds(doc1, doc1.property().isText)
    ], checkSingleProp, {});
    PropertyAssertions.check('Check text range', [
    ...
    PropertyAssertions.check('Check empty range', [
        Arbitraries.arbRangeIds(doc1, doc1.property().isEmptyTag)
    ], checkRangeProp, {});
    PropertyAssertions.check('Check boundary single', [
        Arbitraries.arbIds(doc1, doc1.property().isBoundary)
    ], checkSingleProp, {});
    PropertyAssertions.check('Check boundary range', [
        Arbitraries.arbRangeIds(doc1, doc1.property().isBoundary)
    ], checkRangeProp, {});
    PropertyAssertions.check('Check inline tag single', [
    ...
    PropertyAssertions.check('Check inline tag range', [
        Arbitraries.arbRangeIds(doc1, function (item) {
            return !(doc1.property().isBoundary(item) || doc1.property().isEmptyTag(item) || doc1.property().isText(item));
        })
    ], checkRangeProp, {});
    PropertyAssertions.check('Check any tag range', [
        Arbitraries.arbRangeIds(doc1, Fun.constant(true))
    ], checkRangeProp, {});
});
//# sourceMappingURL=TextZoneTest.js.map