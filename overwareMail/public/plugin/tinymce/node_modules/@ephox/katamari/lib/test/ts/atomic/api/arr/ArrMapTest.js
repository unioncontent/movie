...
UnitTest.test('MapTest', function () {
    ...
    Jsc.property('map id xs = xs', Jsc.array(Jsc.json), function (arr) {
        var output = Arr.map(arr, Fun.identity);
        return Jsc.eq(arr, output);
    });
    Jsc.property('map constant (y) xs = array of y', Jsc.array(Jsc.json), Jsc.json, function (arr, y) {
        var output = Arr.map(arr, Fun.constant(y));
        return Arr.forall(output, function (x) {
            return x === y;
        });
    });
    Jsc.property('mapToObject generates the right number of keys and f(key) = value in object', Jsc.array(Jsc.nestring), Jsc.fun(Jsc.json), function (rawKeys, f) {
