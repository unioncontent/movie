...
UnitTest.test('FoldTest', function () {
    ...
    Jsc.property('foldl concat [ ] xs === reverse(xs)', Jsc.array(Jsc.json), function (arr) {
        var output = Arr.foldl(arr, function (b, a) {
            return [a].concat(b);
        }, []);
        return Jsc.eq(Arr.reverse(arr), output);
    });
    Jsc.property('foldr concat [ ] xs === xs', Jsc.array(Jsc.json), function (arr) {
        var output = Arr.foldr(arr, function (b, a) {
            return [a].concat(b);
        }, []);
        return Jsc.eq(arr, output);
    });
    Jsc.property('foldr concat ys xs === xs ++ ys', Jsc.array(Jsc.json), Jsc.array(Jsc.json), function (xs, ys) {
        var output = Arr.foldr(xs, function (b, a) {
            return [a].concat(b);
        }, ys);
        return Jsc.eq(xs.concat(ys), output);
    });
    Jsc.property('foldl concat ys xs === reverse(xs) ++ ys', Jsc.array(Jsc.json), Jsc.array(Jsc.json), function (xs, ys) {
