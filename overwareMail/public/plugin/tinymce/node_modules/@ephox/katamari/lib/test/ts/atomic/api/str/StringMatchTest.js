...
UnitTest.test('StringMatchTest', function () {
    var check = function (testcase) {
        ...
        assert.eq(!testcase.expected, StringMatch.matches(StringMatch.not(testcase.match), testcase.input));
    };
    ...
    Jsc.property('StringMatch.matches(StringMatch.starts(s1), s1 + s) === true', Jsc.string, Jsc.string, function (s, s1) {
        return Jsc.eq(true, StringMatch.matches(StringMatch.starts(s1, StringMatch.caseInsensitive), s1 + s));
    });
    Jsc.property('StringMatch.matches(StringMatch.contains(s1), s1 + s) === true', Jsc.string, Jsc.string, function (s, s1) {
        return Jsc.eq(true, StringMatch.matches(StringMatch.contains(s1, StringMatch.caseInsensitive), s1 + s));
    });
    Jsc.property('StringMatch.matches(StringMatch.contains(s1), s + s1) === true', Jsc.string, Jsc.string, function (s, s1) {
        return Jsc.eq(true, StringMatch.matches(StringMatch.contains(s1, StringMatch.caseInsensitive), s + s1));
    });
    Jsc.property('StringMatch.matches(StringMatch.contains(s1), s) === s.indexOf(s1)', Jsc.asciistring, Jsc.asciinestring, function (s, s1) {
        return Jsc.eq(s.toLowerCase().indexOf(s1.toLowerCase()) > -1, StringMatch.matches(StringMatch.contains(s1, StringMatch.caseInsensitive), s));
    });
    Jsc.property('StringMatch.matches(StringMatch.contains(s), s) === true', Jsc.string, function (s) {
        return Jsc.eq(true, StringMatch.matches(StringMatch.contains(s, StringMatch.caseInsensitive), s));
    });
    Jsc.property('StringMatch.matches(StringMatch.exact(s), s + s1) === false', Jsc.nestring, Jsc.nestring, function (s, s1) {
        return Jsc.eq(false, StringMatch.matches(StringMatch.exact(s, StringMatch.caseInsensitive), s + s1));
    });
    Jsc.property('StringMatch.matches(StringMatch.exact(s), s1 + s) === false', Jsc.nestring, Jsc.nestring, function (s, s1) {
        return Jsc.eq(false, StringMatch.matches(StringMatch.exact(s, StringMatch.caseInsensitive), s1 + s));
    });
    Jsc.property('StringMatch.matches(StringMatch.exact(s), s) === true', Jsc.nestring, function (s) {
        return Jsc.eq(true, StringMatch.matches(StringMatch.exact(s, StringMatch.caseInsensitive), s));
    });
    Jsc.property('StringMatch.matches(StringMatch.exact(s), s) === false when different case and case-insensitive', Jsc.asciinestring, function (s) {
        return s.toUpperCase() === s.toLowerCase() || Jsc.eq(true, StringMatch.matches(StringMatch.exact(s.toLowerCase(), StringMatch.caseInsensitive), s.toUpperCase()));
    });
    Jsc.property('StringMatch.matches(StringMatch.exact(s), s) === false when different case and case-sensitive', Jsc.asciinestring, function (s) {
        return s.toUpperCase() === s.toLowerCase() || Jsc.eq(false, StringMatch.matches(StringMatch.exact(s.toLowerCase(), StringMatch.caseSensitive), s.toUpperCase()));
    });
    Jsc.property('StringMatch.matches(StringMatch.all(s1), *) === true', Jsc.string, Jsc.string, function (s) {
        return Jsc.eq(true, StringMatch.matches(StringMatch.all(), s));
    });
});
