...
UnitTest.test('ObjFindTest', function () {
    ...
    Jsc.property('the value found by find always passes predicate', Jsc.dict(Jsc.json), Jsc.fun(Jsc.bool), function (obj, pred) {
    ...
    Jsc.property('If predicate is always false, then find is always none', Jsc.dict(Jsc.json), function (obj) {
        var value = Obj.find(obj, Fun.constant(false));
        return value.isNone();
    });
    Jsc.property('If object is empty, find is always none', Jsc.fun(Jsc.bool), function (pred) {
        var value = Obj.find({}, pred);
        return value.isNone();
    });
    Jsc.property('If predicate is always true, then value is always the some(first), or none if dict is empty', Jsc.dict(Jsc.json), function (obj) {
