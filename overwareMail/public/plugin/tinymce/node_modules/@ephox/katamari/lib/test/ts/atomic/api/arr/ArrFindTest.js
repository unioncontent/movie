...
UnitTest.test('ArrFindTest', function () {
    ...
    Jsc.property('the value found by find always passes predicate', Jsc.array(Jsc.json), Jsc.fun(Jsc.bool), function (arr, pred) {
    ...
    Jsc.property('If predicate is always false, then find is always none', Jsc.array(Jsc.json), function (arr) {
        var value = Arr.find(arr, Fun.constant(false));
        return value.isNone();
    });
    Jsc.property('If array is empty, find is always none', Jsc.fun(Jsc.bool), function (pred) {
        var value = Arr.find([], pred);
        return value.isNone();
    });
    Jsc.property('If predicate is always true, then value is always some(first), or none if array is empty', Jsc.array(Jsc.json), function (arr) {
