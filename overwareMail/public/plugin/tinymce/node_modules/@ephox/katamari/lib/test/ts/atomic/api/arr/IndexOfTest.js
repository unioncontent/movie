...
UnitTest.test('IndexOfTest', function () {
    ...
    Jsc.property('indexOf(slice, val) == some(0) if slice starts with val', Jsc.array(Jsc.json), function (arr) {
        return Arr.forall(arr, function (x, i) {
            var index = Arr.indexOf(arr.slice(i), x).getOrDie('should have index');
            return 0 === index;
        });
    });
    Jsc.property('indexOf(unique_arr, val) === some(iterator index)', ArbDataTypes.indexArrayOf(10), function (arr) {
        return Arr.forall(arr, function (x, i) {
            var index = Arr.indexOf(arr, x).getOrDie('should have index');
            return i === index;
        });
    });
    Jsc.property('indexOf of an empty array is none', Jsc.json, function (json) {
        return Arr.indexOf([], json).isNone();
    });
    Jsc.property('indexOf of a [value].concat(array) is some(0)', Jsc.array(Jsc.json), Jsc.json, function (arr, json) {
