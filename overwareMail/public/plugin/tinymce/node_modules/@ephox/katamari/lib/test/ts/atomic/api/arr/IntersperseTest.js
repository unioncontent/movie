...
UnitTest.test('Intersperse', function () {
    ...
    Jsc.property('Length of interspersed = len(arr) + len(arr)-1', Jsc.array(Jsc.json), Jsc.json, function (arr, delimiter) {
        var actual = Jam.intersperse(arr, delimiter);
        var expected = arr.length === 0 ? 0 : arr.length * 2 - 1;
        return Jsc.eq(expected, actual.length);
    });
    Jsc.property('Every odd element matches delimiter', Jsc.array(Jsc.json), Jsc.json, function (arr, delimiter) {
        var actual = Jam.intersperse(arr, delimiter);
        return Arr.forall(actual, function (x, i) {
            return i % 2 === 1 ? Jsc.eq(x, delimiter) : true;
        });
    });
    Jsc.property('Filtering out delimiters (assuming different type to array to avoid removing original array) should equal original', Jsc.array(Jsc.string), Jsc.nat, function (arr, delimiter) {
        var actual = Jam.intersperse(arr, delimiter);
