...
UnitTest.test('FindIndexTest', function () {
    ...
    Jsc.property('the index found by findIndex always passes predicate', Jsc.array(Jsc.json), Jsc.fun(Jsc.bool), function (arr, pred) {
    ...
    Jsc.property('If predicate is always false, then index is always none', Jsc.array(Jsc.json), function (arr) {
        var index = Arr.findIndex(arr, Fun.constant(false));
        return index.isNone();
    });
    Jsc.property('If predicate is always true, then index is always some(0), or none if array is empty', Jsc.array(Jsc.json), function (arr) {
