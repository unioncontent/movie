...
UnitTest.test('ForallTest', function () {
    ...
    Jsc.property('forall of an empty array is true', Jsc.fun(Jsc.bool), function (pred) {
        var output = Arr.forall([], pred);
        return Jsc.eq(true, output);
    });
    Jsc.property('forall of a non-empty array with a predicate that always returns false is false', Jsc.nearray(Jsc.json), function (xs) {
        var output = Arr.forall(xs, Fun.constant(false));
        return Jsc.eq(false, output);
    });
    Jsc.property('forall of a non-empty array with a predicate that always returns true is true', Jsc.nearray(Jsc.json), function (xs) {
        var output = Arr.forall(xs, Fun.constant(true));
        return Jsc.eq(true, output);
    });
    Jsc.property('forall of an array = true if filter(array).length = array.length', Jsc.array(Jsc.json), Jsc.fun(Jsc.bool), function (xs, g) {
