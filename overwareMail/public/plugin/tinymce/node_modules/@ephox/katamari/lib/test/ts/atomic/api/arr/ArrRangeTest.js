...
UnitTest.test('ArrRangeTest', function () {
    ...
    Jsc.property('Range(num, Fun.identity) should Arr.forall(x === index) && length === num', Jsc.nat, function (num) {
        var range = Arr.range(num, Fun.identity);
        return Arr.forall(range, function (x, i) {
            return x === i;
        }) && range.length === num;
    });
    Jsc.property('Range(num, Fun.constant(c)) should Arr.forall(x === c) && length === num', Jsc.nat, Jsc.json, function (num, c) {
