...
UnitTest.test('ObjResolverTest', function () {
    ...
    var testSpecs = function () {
        ...
        Jsc.property('Checking that creating a namespace (forge) from an obj will enable that value to be retrieved by resolving (path)', 
        ...
        Jsc.dict(Jsc.dict(Jsc.constant({}))))), Jsc.nearray(Jsc.asciinestring), Jsc.asciinestring, Jsc.asciinestring, function (dict, parts, field, newValue) {
            var created = Resolve.forge(parts, dict);
            created[field] = newValue;
            var resolved = Resolve.path(parts.concat([field]), dict);
            return checkEq('Checking that namespace works with resolve', newValue, resolved);
        });
    };
    testNamespace();
    testResolve();
