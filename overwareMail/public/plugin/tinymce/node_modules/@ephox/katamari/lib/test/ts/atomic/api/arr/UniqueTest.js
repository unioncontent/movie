...
UnitTest.test('Unique', function () {
    ...
    Jsc.property('The result of a unique test should contain no duplicates', Jsc.array(Jsc.nestring), function (arr) {
        var unique = Unique.stringArray(arr);
        return Arr.forall(unique, function (x, i) {
            return !Arr.contains(unique.slice(i + 1), x);
        });
    });
    Jsc.property('Unique is idempotent (assuming sorted)', Jsc.array(Jsc.nestring), function (arr) {
