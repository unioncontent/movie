...
UnitTest.asynctest('Node Change Test', function (success, failure) {
    ...
    var checkAriaVoice = function (spec) {
        return Step.async(function (next, err) {
            ...
            Pipeline.async({}, [
                Step.sync(function () {
                    assert.eq(isPresent(spec.selector), false, 'speech token should not be present in DOM.');
                    spec.action(p, spec.speech);
                    assert.eq(isPresent(spec.selector), true, 'speech token should be present in the DOM.');
                }),
                GeneralSteps.waitForPredicate('speech token to be removed from DOM', 400, 2000, function () { return !isPresent(spec.selector); })
            ], function () {
                cleanup();
                next();
            }, function () {
...
//# sourceMappingURL=AriaVoiceRemovedTest.js.map