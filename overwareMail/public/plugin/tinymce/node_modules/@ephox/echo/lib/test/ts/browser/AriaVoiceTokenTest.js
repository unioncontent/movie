...
UnitTest.test('AriaRegisterTest', function () {
    ...
    var matchToString = function (subject) {
        return StringMatch.cata(subject, function (value) { return 'startsWith(' + value + ')'; }, function (regex) { return 'pattern(' + regex + ')'; }, function (value) { return 'contains(' + value + ')'; }, function (value) { return 'exact(' + value + ')'; }, function () { return 'all'; }, function (sm) { return 'not(' + matchToString(sm) + ')'; });
    };
    var checkValues = function (label, type, expected, getActual) {
        Obj.each(expected, function (exp, k) {
            exp.fold(function () {
                getActual(k).each(function (v) {
                    assert.fail('Test: ' + label + '. Token should not have: ' + k + '=' + v);
                });
            }, function (expMatch) {
                getActual(k).fold(function () {
    ...
    checkDynamic('shouting "fire"', {
        ...
        'left': oExact('-9999px'),
        'display': Option.none()
    }, AriaVoice.shout, 'fire');
    checkStatic('describing: a long long time ago', {
        ...
        display: oExact('none')
    }, AriaVoice.describe, 'a long long time ago');
});
//# sourceMappingURL=AriaVoiceTokenTest.js.map