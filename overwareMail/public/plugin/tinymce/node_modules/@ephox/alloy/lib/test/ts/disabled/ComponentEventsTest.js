...
UnitTest.test('ComponentEventsTest', function () {
    ...
    var checkErr = function (expectedPart, info, behaviours) {
        ResultAssertions.checkErr('Checking error combined events', expectedPart, function () {
            return ComponentEvents.combine(info, info.eventOrder(), behaviours, base);
        });
    };
    var check = function (expected, info, behaviours) {
        ResultAssertions.checkVal('Checking value of combined events', function () {
            store.clear();
            return ComponentEvents.combine(info, info.eventOrder(), behaviours, base);
        }, function (value) {
    ...
    Logger.sync('Testing no behaviours', function () {
        check([
            'base.0'
        ], eo({}), []);
    });
    Logger.sync('Testing 1 behaviour with 1 event', function () {
        check([
            'base.0',
            'a.one'
        ], eo({}), [
    ...
    Logger.sync('Testing 1 behaviour with 2 events', function () {
        check([
            'base.0',
            'a.one',
            'a.two'
    ...
    Logger.sync('Testing complex behaviour with many events and incomplete ordering', function () {
        checkErr('event ordering', eo({}), [
            behaviour('a.behaviour', {
                'event.1': EventHandler.nu({
                    run: store.adder('a.one')
                }),
    ...
    Logger.sync('Testing complex behaviour with many events and not quite complete ordering', function () {
        checkErr('entry for b.behaviour', eo({
            'event.3': ['a.behaviour']
        }), [
            behaviour('a.behaviour', {
                'event.1': EventHandler.nu({
    ...
    Logger.sync('Testing complex behaviour with many events and *complete* ordering', function () {
        check(['base.0', 'a.one', 'a.two', 'b.three.abort', 'event.3.stop'], eo({
...
//# sourceMappingURL=ComponentEventsTest.js.map