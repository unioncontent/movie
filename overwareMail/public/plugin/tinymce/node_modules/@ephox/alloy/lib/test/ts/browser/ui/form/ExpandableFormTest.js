...
UnitTest.asynctest('ExpandableFormTest', function () {
    ...
    GuiSetup.setup(function (store, doc, body) {
        var pMinimal = ExpandableForm.parts().minimal(Form.sketch(function (parts) {
            return {
                dom: {
                    tag: 'div',
        ...
        var pExtra = ExpandableForm.parts().extra(Form.sketch(function (parts) {
            return {
                dom: {
                    tag: 'div',
        ...
        var me = GuiFactory.build(ExpandableForm.sketch({
            dom: {
                tag: 'div'
            },
            components: [
        ...
        return [
            ...
            Logger.t('Checking form after set', helper.sAssertRep({
                'form.ant': 'first.set',
                'form.bull': 'select-b-set'
            })),
            Logger.t('Retrieve the ant field directly and check its value (in minimal)', GeneralSteps.sequence([
                Step.sync(function () {
            ...
            Logger.t('Retrieve the bull field directly and check its value (in extra)', GeneralSteps.sequence([
                Step.sync(function () {
            ...
            Logger.t('Checking the representing value is still the same when collapsed', helper.sAssertRep({
                'form.ant': 'first.set',
                'form.bull': 'select-b-set'
            })),
            // Expand the form, focus the part in the extra, and click on expand again and check focus is still in dialog
            Logger.t('Expanding form, focusing part in extra, clicking on expand, and checking focus still in dialog', GeneralSteps.sequence([
                ...
                Keyboard.sKeydown(doc, Keys.enter(), {}),
                Waiter.sTryUntil('Waiting until it has stopped growing', UiFinder.sNotExists(gui.element(), '.expandable-growing'), 100, 10000),
                Keyboard.sKeydown(doc, Keys.tab(), {}),
                FocusTools.sTryOnSelector('Focus should move onto select', doc, 'select'),
                Mouse.sClickOn(gui.element(), '.test-expander-button'),
                Waiter.sTryUntil('Waiting until it has stopped shrinking', UiFinder.sNotExists(gui.element(), '.expandable-shrinking'), 100, 10000),
...
//# sourceMappingURL=ExpandableFormTest.js.map