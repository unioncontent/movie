...
var toInfo = function (spec) {
    return ValueSchema.asStruct('custom.definition', ValueSchema.objOfOnly([
        ...
        FieldSchema.field('eventOrder', 'eventOrder', FieldPresence.mergeWith({
...
var toDefinition = function (info) {
    var base = {
        ...
        attributes: Merger.deepMerge(getUid(info), info.dom().attributes()),
        styles: info.dom().styles(),
        domChildren: Arr.map(info.components(), function (comp) { return comp.element(); })
    };
    return DomDefinition.nu(Merger.deepMerge(base, info.dom().innerHtml().map(function (h) { return Objects.wrap('innerHtml', h); }).getOr({}), info.dom().value().map(function (h) { return Objects.wrap('value', h); }).getOr({})));
};
...
export { toInfo, toDefinition, toModification, toApis, toEvents };
//# sourceMappingURL=CustomDefinition.js.map