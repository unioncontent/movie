...
UnitTest.asynctest('Browser Test: events.AttachingEventTest', function () {
    ...
    var gui = Gui.takeover(GuiFactory.build(Container.sketch({
        dom: {
            ...
            Container.sketch({
                events: AlloyEvents.derive([
                    AlloyEvents.runOnAttached(function (comp, simulatedEvent) {
                        simulatedEvent.stop();
                        var parent = Traverse.parent(comp.element()).getOrDie('At attachedToDom, a DOM parent must exist');
                        store.adder('attached-to:' + Attr.get(parent, 'class'))();
                    }),
                    AlloyEvents.runOnDetached(function (comp, simulatedEvent) {
                        simulatedEvent.stop();
                        var parent = Traverse.parent(comp.element()).getOrDie('At detachedFromDom, a DOM parent must exist');
    ...
    Pipeline.async({}, [
        Step.sync(function () {
            RawAssertions.assertEq('Checking that the component has no size', 0, wrapper.element().dom().getBoundingClientRect().width);
        }),
        ...
        Step.wait(500),
        Step.sync(function () {
            RawAssertions.assertEq('Even though added to system, not added to DOM yet so still size 0', 0, wrapper.element().dom().getBoundingClientRect().width);
        }),
        ...
        Step.sync(function () {
            RawAssertions.assertEq('Now added to the DOM, so should have size 100', 100, wrapper.element().dom().getBoundingClientRect().width);
        }),
...
//# sourceMappingURL=AttachingEventTest.js.map