...
UnitTest.asynctest('Browser Test: ui.slider.SliderTest', function () {
    ...
    GuiSetup.setup(function (store, doc, body) {
        return GuiFactory.build(Slider.sketch({
            dom: {
                tag: 'div',
                classes: ['slider-test']
            },
        ...
        var cCheckThumbAtLeft = Chain.op(function (parts) {
            RawAssertions.assertEq('Thumb (' + parts.thumbRect.left + '->' + parts.thumbRect.right +
                '), Left-Edge: (' + parts.ledgeRect.left + '->' + parts.ledgeRect.right + ')', true, parts.ledgeRect.right > parts.thumbRect.left && parts.ledgeRect.left < parts.thumbRect.left);
        });
        var cCheckThumbAtRight = Chain.op(function (parts) {
            RawAssertions.assertEq('Thumb (' + parts.thumbRect.left + '->' + parts.thumbRect.right +
                '), Right-Edge: (' + parts.redgeRect.left + '->' + parts.redgeRect.right + ')', true, parts.redgeRect.left < parts.thumbRect.right && parts.ledgeRect.left < parts.redgeRect.left);
        });
        var cCheckThumbPastRight = Chain.op(function (parts) {
            RawAssertions.assertEq('Checking thumb past end of spectrum', true, parts.thumbRect.left > parts.spectrumRect.right);
        });
        var cCheckThumbBeforeLeft = Chain.op(function (parts) {
            RawAssertions.assertEq('Checking thumb before start of spectrum', true, parts.thumbRect.right < parts.spectrumRect.left);
        });
        ...
        return [
            ...
            Logger.t('Initial-Value: Checking that the thumb now overlaps the right edge at max', Waiter.sTryUntil('Initial load can take a while', Chain.asStep({}, [
                cGetParts,
                cCheckThumbAtRight,
                cCheckValue(200)
            ]), 100, 1000)),
            ...
            Logger.t('Checking that the thumb now overlaps the left edge at min', Chain.asStep({}, [
                cGetParts,
                cCheckThumbAtLeft,
                cCheckValue(50)
            ])),
            ...
            Logger.t('Checking that the thumb now overlaps the right edge at max', Chain.asStep({}, [
                cGetParts,
                cCheckThumbAtRight,
                cCheckValue(200)
            ])),
            Logger.t('Focus the gradient', Chain.asStep({}, [
                cGetParts,
            ...
            Logger.t('Checking that the thumb is past the max', Chain.asStep({}, [
                cGetParts,
                cCheckThumbPastRight,
                cCheckValue(201)
            ])),
            Keyboard.sKeydown(doc, Keys.right(), {}),
            Logger.t('Pressed right at right edge. Checking that the thumb is still past the max and value has not changed', Chain.asStep({}, [
                cGetParts,
                cCheckThumbPastRight,
                cCheckValue(201)
            ])),
            Keyboard.sKeydown(doc, Keys.left(), {}),
            Logger.t('Pressed left at the right edge. Thumb should be at max', Chain.asStep({}, [
                cGetParts,
                cCheckThumbAtRight,
                cCheckValue(200)
            ])),
            ...
            Keyboard.sKeydown(doc, Keys.left(), {}),
            Logger.t('Checking that the thumb is before the min', Chain.asStep({}, [
                cGetParts,
                cCheckThumbBeforeLeft,
                cCheckValue(49)
            ])),
            Keyboard.sKeydown(doc, Keys.left(), {}),
            Logger.t('Checking that the thumb is *still* before the min', Chain.asStep({}, [
                cGetParts,
                cCheckThumbBeforeLeft,
                cCheckValue(49)
            ])),
            Keyboard.sKeydown(doc, Keys.right(), {}),
            Logger.t('Checking that the thumb is at the left edge', Chain.asStep({}, [
