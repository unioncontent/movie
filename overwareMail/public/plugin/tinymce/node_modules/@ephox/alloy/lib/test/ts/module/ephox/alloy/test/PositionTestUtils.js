...
var cTestPopupInSink = function (label, sinkName) {
    return Chain.control(NamedChain.bundle(function (data) {
        var sink = data[sinkName];
        var inside = Sinks.isInside(sink, data.popup);
        return inside ? Result.value(data) : Result.error(new Error('The popup does not appear within the ' + label + ' sink container'));
...
var cTestSink = function (label, sinkName) {
    return ChainUtils.cLogging(label, [
        cAddPopupToSink(sinkName),
        cTestPopupInSink(sinkName, sinkName)
    ]);
};
var cScrollDown = function (componentName, amount) {
    return ChainUtils.cLogging('Adding margin to ' + componentName + ' and scrolling to it', [
...
//# sourceMappingURL=PositionTestUtils.js.map