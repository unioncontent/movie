...
UnitTest.asynctest('Browser Test: ui.touch.TouchMenuTest', function () {
    ...
    GuiSetup.setup(function (store, doc, body) {
        ...
        return GuiFactory.build(TouchMenu.sketch({
            dom: {
                tag: 'div',
        ...
        return [
            ...
            Waiter.sTryUntil('Waiting for menu to disappear', UiFinder.sNotExists(gui.element(), '[role="menu"]'), 100, 1000),
            Step.sync(function () {
                store.assertEq('Checking no messages', []);
                var rect = component.element().dom().getBoundingClientRect();
                fireTouchstart(component.element(), rect.x, rect.y);
            ...
            Waiter.sTryUntil('Waiting until menu appears', UiFinder.sExists(gui.element(), '[role=menu]'), 100, 1000),
            ...
            sAssertMenuStructure('Checking menu structure with hover over the touch button (so nothing selected)', ApproxStructure.build(function (s, str, arr) {
                return s.element('div', {
                    children: [
                        s.element('div', {
                            classes: [arr.not('test-selected-item')]
            ...
            sFireTouchmoveOn(component.element(), '.touch-menu-button'),
            store.sAssertEq('Hover on should not fire again because hover off has not yet', ['onHoverOn', 'onHoverOff', 'onHoverOn']),
            // Entire component, so inside the menu part
            sFireTouchmoveOn(component.element(), '.touch-menu'),
            store.sAssertEq('Hover off should fire because nothing under "touch"', ['onHoverOn', 'onHoverOff', 'onHoverOn', 'onHoverOff']),
            sAssertMenuStructure('Checking menu structure with hover over nothing (so nothing selected)', ApproxStructure.build(function (s, str, arr) {
                return s.element('div', {
                    children: [
                        s.element('div', {
                            classes: [arr.not('test-selected-item')]
...
//# sourceMappingURL=TouchMenuTest.js.map