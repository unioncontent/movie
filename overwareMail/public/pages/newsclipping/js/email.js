      ...
      if(data.status){
        ...
        $.each(data.result.news, function(index, value) {
          ...
          if(value.news_type == "4"){
            ...
            var pageHtml = '';
            if(value.media_page != null && value.media_page != ''){
              pageHtml = '<td style="padding:10px 0;border:1px solid #f4f4f4;border-top:0;text-align:center;color:#666;">'+value.media_page+'</td>';
            }
            var html = '<tr><td style="padding: 10px 0px 5px 7px;border-bottom: 1px solid #f4f4f4;"><div style="max-width: 538px;display: inline-block;text-overflow:ellipsis;white-space: initial;"><a style="text-decoration:none !important;color:black;font-weight:bold;" href="'+value.url+'" target="_blank">'+siteLogoHtml+value.media_title+replyHtml+'</a><i class="fas fa-times news-delete" data-idx="'+value.media_idx+'" data-type="news_mail" data-str="media_idx"></i></div></td><td style="padding:10px 0;border:1px solid #f4f4f4;border-top:0;text-align:center;color:#666;">'+value.media_name+'</td>'+pageHtml+'<td style="padding: 10px 0;border-bottom:1px solid #f4f4f4;text-align:center;color:#666;">'+value.writeDate+'</td></tr>'
            ...
            html += '<div class="news-content" data-idx="'+value.media_idx+'" data-type="media" style="font-size:12px;width: 700px;display: inline-block;"><div class="title" style="float:left;max-width:533px;display:inline-block;white-space: inherit;"><a style="text-decoration:none !important;color:black;font-weight:bold;" href="'+value.url+'" target="_blank">'+siteLogoHtml+value.media_title+replyHtml+'</a></div><div clacss="info" style="color:#666;float:right;"><span class="media-name">'+value.media_name+'</span>'+pageHtml+'<span class="bar" style="display:inline-block;overflow:hidden;width:0;height:11px;margin:-1px 5px 1px 4px;border-left:1px solid #eaeaea;vertical-align: middle;"></span><span class="date">'+value.writeDate+'</span><i class="fas fa-times news-delete" data-idx="'+value.media_idx+'" data-type="news_mail" data-str="media_idx"></i></div>';
            html +='</div></div>';
        ...
        $.each(obj, function(index, value) {
          ...
          var html ='<ul class="relation_list" data-midx="'+index+'" style="list-style: none;padding:0;margin: 0;margin-bottom:  0px;margin-left: 10px;line-height: 18px;position: relative;font-size: 12px;"></ul>';
          $('div[data-idx='+index+']').after(html);
