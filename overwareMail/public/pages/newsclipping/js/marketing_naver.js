...
$('#searchBtn').on('click',function(){
  var url = '/marketing/naver?page=1';
...
function ajaxGetPageList(param){
  ...
  $('.preloader3').show();
  $.ajax({
    url: '/marketing/naver/getNextPage',
    type: 'post',
    data: param,
    error: function(request,status,error){
      console.log('code:'+request.status+'\n'+'message:'+request.responseText+'\n'+'error:'+error);
    ...
    success: function(data){
      ...
      data.result.list.forEach(function(item,idx){
        ...
        var html = '<tr>\
        <input type="hidden" id="idx" value="'+item.portal_idx+'" />\
        <td>\
          ...
          <td>\
            <a href="'+item.url+'" target="_blank" title="'+item.portal_title+'">\
              <div class="title_nobr">'+item.portal_title+'</div>\
            </a>\
          </td>\
          <td><input type="text" placeholder="대표제목"  id="title" value="'+((item.portal_subtitle  == null) ? '' :item.portal_subtitle)+'"class="form-control title_input"/></td>\
        </tr>';
