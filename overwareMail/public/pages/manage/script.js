  ...
  if (moduleVal == '2' && typeVal != '0' && tdEles.eq(8).text() == '-') {
    $.ajax({
      ...
      success: function(data) {
        console.log(data);
        var msg = (data.status == 112)
          ? "예약메일의 상태가 발송대기 중이므로 삭제해도 예약발송취소는 불가합니다."
          : "";
        swal({
          title: "삭제하시겠습니까?",
          text: msg,
...
function deleteAjax(param) {
  ...
  $.ajax({
    ...
    success: function(data) {
      if (data.status) {
        swal("SUCCESS!", '삭제 되었습니다.', "success").then(function(value) {
          reloadPage();
        });
      } else {
...
function sendMail(idx) {
  $.ajax({
    ...
    success: function(data) {
      if (data.status) {
        swal("SUCCESS!", '메일이 발송 되었습니다.', "success").then(function(value) {
          reloadPage();
        });
      } else {
        swal("ERROR!", '메일발송을 실패 하였습니다.', "error");
...
function resendMail(idx) {
  $.ajax({
    ...
    success: function(data) {
      if (data.status) {
        swal("SUCCESS!", '메일이 재발송 되었습니다.', "success").then(function(value) {
          reloadPage();
        });
      } else {
        swal("ERROR!", '재발송 실패 하였습니다.', "error");
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      data.result.list.forEach(function(item, idx) {
        ...
        <td>' + item.M_sender + '</td>\
        <td><div class="num_nobr">' + (
          (item.M_invitation == '0' && item.M_seq_number != '')
          ? (item.M_seq_number + '차')
          : '-') + '</div></td>\
        <td>' + recipiStr + '</td>\
      ...
      if (pageCount > 1) {
        ...
        for (var index = pRCnt * pageSize + 1; index < (pRCnt + 1) * pageSize + 1; index++) {
          var active = (data.result.currentPage == index)
            ? "active"
            : "";
...
function startSetting() {
  ...
  var optionSet1 = {
    startDate: (sDate != '' && sDate != null)
      ? moment(new Date(sDate))
      : moment().subtract(7, 'd'),
    endDate: (eDate != '' && eDate != null)
      ? moment(new Date(eDate))
      : moment(),
    showDropdowns: true,
