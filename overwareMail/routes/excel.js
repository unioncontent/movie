...
router.get('/',async function(req, res) {
  ...
  ws.cell(1,3).string('키워드').style(tStyle);
  ...
  ws.cell(1,8).string('성명').style(tStyle);
  ws.cell(1,9).string('이메일').style(tStyle);
  ws.cell(1,10).string('발송날짜').style(tStyle);
  ws.cell(1,11).string('작성날짜').style(tStyle);
  ws.cell(1,12).string('개봉날짜').style(tStyle);
  ws.cell(1,13).string('발송결과').style(tStyle);
  var row = 0;
  var MSGIDStr;
  await asyncForEach(sucess, async (item, index, array) => {
    ...
    ws.cell(row,3).string(item.M_keyword_str);
    ws.cell(row,4).string(item.M_mail_type);
    ws.cell(row,5).string(((item.M_invitation == '0')?(item.M_seq_number+'차'):'인비테이션'));
    ws.cell(row,6).string(item.M_subject);
    ws.cell(row,7).string(item.M_ptitle);
    ws.cell(row,8).string(item.EMTONAME);
    ws.cell(row,9).string(item.EMTOADDRESS);
    ws.cell(row,10).string(moment(item.SENDTIME).format('YYYY-MM-DD HH:mm:ss'));
    ws.cell(row,11).string(moment(item.GENDATE).format('YYYY-MM-DD HH:mm:ss'));
    ws.cell(row,12).string(openDate);
    ws.cell(row,13).string('성공');
  });
  ...
  ws2.column(13).setWidth(30);
  ws2.cell(1,1).string('NO').style(tStyle);
  ws2.cell(1,2).string('발송타입').style(tStyle);
  ws2.cell(1,3).string('키워드').style(tStyle);
  ws2.cell(1,4).string('메일타입').style(tStyle);
  ws2.cell(1,5).string('차수').style(tStyle);
  ws2.cell(1,6).string('제목').style(tStyle);
  ws2.cell(1,7).string('회사').style(tStyle);
  ws2.cell(1,8).string('성명').style(tStyle);
  ws2.cell(1,9).string('이메일').style(tStyle);
  ws2.cell(1,10).string('발송날짜').style(tStyle);
  ws2.cell(1,11).string('작성날짜').style(tStyle);
  ws2.cell(1,12).string('발송결과').style(tStyle);
  ws2.cell(1,13).string('오류내용').style(tStyle);
  row = 0;
  await asyncForEach(fail, async (item, index, array) => {
    ...
    ws2.cell(row,3).string(item.M_keyword_str);
    ws2.cell(row,4).string(item.M_mail_type);
    ws2.cell(row,5).string(((item.M_invitation == '0')?(item.M_seq_number+'차'):'인비테이션'));
    ws2.cell(row,6).string(item.M_subject);
    ws2.cell(row,7).string(item.M_ptitle);
    ws2.cell(row,8).string(item.EMTONAME);
    ws2.cell(row,9).string(item.EMTOADDRESS);
    ws2.cell(row,10).string(moment(item.SENDTIME).format('YYYY-MM-DD HH:mm:ss'));
    ws2.cell(row,11).string(moment(item.GENDATE).format('YYYY-MM-DD HH:mm:ss'));
    ws2.cell(row,12).string('실패');
    ws2.cell(row,13).string(settingErrorMsg(item.FINALRESULT));
  });
