...
var period = {
  ...
  selectReservationCount: async function(admin,n_idx){
    ...
    var userResult = await funDB.getResult('d',sql,[((admin==null)?n_idx:admin),n_idx]);
    ...
    var keywordResult  = await funDB.getResult('d',sql,n_idx);
    ...
    var count = await funDB.getResult('m',sql,[]);
    if(count.length == 0){
      return 0;
    }
    else{
  ...
  getNewsCount: async function(param){
    ...
    var result = await funDB.getResult('o',sql,pValue);
    if(result.length == 0){
      return 0;
    }
    return result[0].c || 0;
  ...
  getPeriodMediaNReporterCount: async function(param){
    ...
    var result = await funDB.getResult('d',sql,value);
    if(result.length == 0){
