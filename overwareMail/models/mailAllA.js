...
var mailAllA = {
  ...
  delete_mail: async function(M_a_id){
    var sql = 'update m_mail_all_a set M_delete = 1, M_senddate = now() where M_a_id=?';
    try {
      await funDB.getResult('o',sql,[M_a_id]);
    } catch (e) {
      console.log(e)
    } finally {
      return await funDB.getResult('d',sql,[M_a_id]);
    }
  },
