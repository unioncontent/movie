...
public class MarketingController {
	...
	public List<GraphVO> graphOne(Model model, String success, String url, String url2,SearchFv fv,SearchNv nv, String Mcreate, String Mcreate2) throws ParseException {
		...
		for(int i = 0; i < 24; i++) {
			...
			FvVO fvVO = new FvVO();
			fv.setUrl(url);
			if (snsService.fvlistGraph(fv).get(i+1) != null) {
			...
			viewList.add(fvVO);
			logger.info("viewList: " + viewList.get(i).getView_cnt() + "/ 번호:" + i++);
		}
		...
		for(int i = 0; i < 24; i++) {
			...
			fv.setUrl(url2);
			if(snsService.fvlistGraph(fv).get(i+1)!=null){
				view2=(snsService.fvlistGraph(fv).get(i+1).getView_cnt() - snsService.fvlistGraph(fv).get(i).getView_cnt());
			    reply2=(snsService.fvlistGraph(fv).get(i+1).getReply_cnt() - snsService.fvlistGraph(fv).get(i).getReply_cnt());
			    like2 =(snsService.fvlistGraph(fv).get(i+1).getLike_cnt() - snsService.fvlistGraph(fv).get(i).getLike_cnt());
			}
			
			fvVO2.setView_cnt(view2);
	        fvVO2.setReply_cnt(reply2);
	        fvVO2.setLike_cnt(like2);
			viewList2.add(fvVO2);
			logger.info("viewList2: " + viewList2.get(i).getView_cnt());
		}
